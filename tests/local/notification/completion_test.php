...
final class completion_test extends \advanced_testcase {
    ...
    public function test_notify_users() {
        ...
        $now = time();
        $allocation1->timecompleted = $now - completion::TIME_CUTOFF + 100;
        $DB->update_record('enrol_programs_allocations', $allocation1);
        $allocation2->timecompleted = $now - completion::TIME_CUTOFF - 100;
        ...
        completion::notify_users($program1, null);
        ...
        $this->setCurrentTimeStart();
        $sink = $this->redirectMessages();
        completion::notify_users(null, $user1);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
        ...
        $this->setCurrentTimeStart();
        $sink = $this->redirectMessages();
        completion::notify_users(null, null);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
