...
final class local_management_test extends \advanced_testcase {
    ...
    public function test_get_program_search_query() {
        global $DB;
        ...
        $programids = $DB->get_fieldset_sql("SELECT p.* FROM {enrol_programs_programs} AS p WHERE $search ORDER BY p.id ASC", $params);
        $this->assertSame([$program1->id], $programids);

        list($search, $params) = management::get_program_search_query(null, 'First', '');
        $programids = $DB->get_fieldset_sql("SELECT * FROM {enrol_programs_programs} WHERE $search ORDER BY id ASC", $params);
        $this->assertSame([$program1->id], $programids);
