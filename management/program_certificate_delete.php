...
$currenturl = new moodle_url('/enrol/programs/management/program_certificate_delete.php', ['id' => $id]);

management::setup_program_page($currenturl, $context, $program);

$cert = $DB->get_record('enrol_programs_certs', ['programid' => $program->id], '*', MUST_EXIST);

$current = clone($cert);
$current->id = $current->programid;

$form = new \enrol_programs\local\form\program_certificate_delete(null, ['data' => $current, 'context' => $context]);
...
if ($data = $form->get_data()) {
    \enrol_programs\local\certificate::delete_program_certificate($data->id);
