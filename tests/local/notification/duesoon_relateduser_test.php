...
/**
 * Program duesoon_relateduser soon notification test.
 *
 * @group      openlms
 ...
 *
 * @covers \enrol_programs\local\notification\duesoon_relateduser
 */
final class duesoon_relateduser_test extends \advanced_testcase {
    ...
    public function test_notify_users() {
        ...
        $now = time();
        $allocation1->timedue = $now - duesoon_relateduser::TIME_CUTOFF + 100;
        $DB->update_record('enrol_programs_allocations', $allocation1);
        $allocation2->timedue = $now - duesoon_relateduser::TIME_CUTOFF - 100;
        $DB->update_record('enrol_programs_allocations', $allocation2);
        $allocation3->timedue = $now + duesoon_relateduser::get_time_soon() - 100;
        $DB->update_record('enrol_programs_allocations', $allocation3);
        $allocation4->timedue = $now + duesoon_relateduser::get_time_soon() + 100;
        $DB->update_record('enrol_programs_allocations', $allocation4);
        $allocation5->timedue = $now + duesoon_relateduser::get_time_soon() - 100;
        $DB->update_record('enrol_programs_allocations', $allocation5);
        $allocation6->timedue = $now + duesoon_relateduser::get_time_soon() - 100;
        $DB->update_record('enrol_programs_allocations', $allocation6);
        $generator->create_program_notification(['notificationtype' => 'duesoon_relateduser', 'programid' => $program1->id]);
        $generator->create_program_notification(['notificationtype' => 'duesoon_relateduser', 'programid' => $program2->id]);

        $this->setCurrentTimestart();
        $sink = $this->redirectMessages();
        duesoon_relateduser::notify_users($program1, null);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
        $this->assertSame('Program completion is expected soon for user ' . fullname($user3), $message->subject);
        ...
        $this->assertSame('duesoon_relateduser_notification', $message->eventtype);
        $this->assertSame($program1->fullname, $message->contexturlname);
        $this->assertTimeCurrent(notification_manager::get_timenotified($user3->id, $program1->id, 'duesoon_relateduser'));
        $this->assertNull(notification_manager::get_timenotified($user1->id, $program1->id, 'duesoon_relateduser'));

        $this->setCurrentTimestart();
        $sink = $this->redirectMessages();
        duesoon_relateduser::notify_users(null, $user1);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
        $this->assertSame('Program completion is expected soon for user ' . fullname($user1), $message->subject);
        $this->assertSame('-10', $message->useridfrom);
        $this->assertSame($related1->id, $message->useridto);
        $this->assertSame('enrol_programs', $message->component);
        $this->assertSame('duesoon_relateduser_notification', $message->eventtype);
        $this->assertSame($program2->fullname, $message->contexturlname);
        $this->assertTimeCurrent(notification_manager::get_timenotified($user1->id, $program2->id, 'duesoon_relateduser'));
        $this->assertNull(notification_manager::get_timenotified($user2->id, $program2->id, 'duesoon_relateduser'));

        $this->setCurrentTimestart();
        $sink = $this->redirectMessages();
        duesoon_relateduser::notify_users(null, null);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
        $this->assertSame('Program completion is expected soon for user ' . fullname($user2), $message->subject);
        $this->assertSame('-10', $message->useridfrom);
        $this->assertSame($related2->id, $message->useridto);
        $this->assertSame('enrol_programs', $message->component);
        $this->assertSame('duesoon_relateduser_notification', $message->eventtype);
        $this->assertSame($program2->fullname, $message->contexturlname);
        $this->assertTimeCurrent(notification_manager::get_timenotified($user2->id, $program2->id, 'duesoon_relateduser'));
