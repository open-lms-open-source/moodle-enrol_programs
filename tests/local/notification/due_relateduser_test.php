...
/**
 * Program due notification for related users test.
 *
 * @group      openlms
 ...
 *
 * @covers \enrol_programs\local\notification\due_relateduser
 */
final class due_relateduser_test extends \advanced_testcase {
    ...
    public function test_notify_users() {
        ...
        $now = time();
        $allocation1->timedue = $now - due_relateduser::TIME_CUTOFF + 100;
        ...
        $generator->create_program_notification(['notificationtype' => 'due_relateduser', 'programid' => $program1->id]);
        $generator->create_program_notification(['notificationtype' => 'due_relateduser', 'programid' => $program2->id]);

        $this->setCurrentTimestart();
        $sink = $this->redirectMessages();
        due_relateduser::notify_users($program1, null);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
        $this->assertSame('Program completion was expected for user ' . fullname($user1), $message->subject);
        ...
        $this->assertSame('due_relateduser_notification', $message->eventtype);
        $this->assertSame($program1->fullname, $message->contexturlname);
        $this->assertTimeCurrent(notification_manager::get_timenotified($user1->id, $program1->id, 'due_relateduser'));
        $this->assertNull(notification_manager::get_timenotified($user2->id, $program1->id, 'due_relateduser'));

        $this->setCurrentTimestart();
        $sink = $this->redirectMessages();
        due_relateduser::notify_users(null, $user1);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
        ...
        $this->assertTimeCurrent(notification_manager::get_timenotified($user1->id, $program2->id, 'due_relateduser'));
        $this->assertNull(notification_manager::get_timenotified($user2->id, $program2->id, 'due_relateduser'));

        $this->setCurrentTimestart();
        $sink = $this->redirectMessages();
        due_relateduser::notify_users(null, null);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
        $this->assertSame('Program completion was expected for user ' . fullname($user2), $message->subject);
        $this->assertSame('-10', $message->useridfrom);
        $this->assertSame($related2->id, $message->useridto);
        $this->assertSame('enrol_programs', $message->component);
        $this->assertSame('due_relateduser_notification', $message->eventtype);
        $this->assertSame($program2->fullname, $message->contexturlname);
        $this->assertTimeCurrent(notification_manager::get_timenotified($user2->id, $program2->id, 'due_relateduser'));
