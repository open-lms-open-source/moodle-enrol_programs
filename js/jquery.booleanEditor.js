...
//        ]
        ...
        $("ul:first > li", editor).each(function () {
            ...
            if (r.hasClass('group')) {
                ...
                expression += [
                    ...
                    '<span class="comp-op"> ' + $("select[name='comparison-operator'] option:selected", r).val() + ' </span>',
    ...
    function getRuleConditionElement(config) {
        ...
        if (config.group) {
            ...
            cond_block_content
                ...
                //.append($('<span>').addClass("comp-op").text('=')) // MUHC
    ...
    function createButtonPannel() {
        var buttonPannel = $('<div class="button-pannel"></div>')
            .append($('<button type="button" class="button-add-cond">'+ M.util.get_string('addcondition', 'enrol_programs') +'</button>')
                .click(function () {
                    addNewConditionAfter($(this).parents('li').get(0));
                })
            )
            .append($('<button type="button" class="button-add-group">'+ M.util.get_string('addgroup', 'enrol_programs') +'</button>')
                .click(function () {
                    addNewGroupAfter($(this).parents('li').get(0));
                })
            )
            .append($('<button type="button" class="button-del-cond">'+ M.util.get_string('deletecondition', 'enrol_programs') +'</button>')
                .click(function () {
